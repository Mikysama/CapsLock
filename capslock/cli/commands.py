...
COMMAND_TREE = (
    ...
    CommandSpec(
        ...
        (
            ...
            CommandSpec("/memory policy", "设置 off/review/automatic 提取策略", "memory", accepts_arguments=True),
            CommandSpec("/memory recall", "启用或禁用自动召回", "memory", accepts_arguments=True),
            CommandSpec("/memory candidates", "查看记忆候选审核队列", "memory", accepts_arguments=True),
            CommandSpec("/memory candidate", "查看、审核、拒绝或清除候选", "memory", accepts_arguments=True),
            CommandSpec("/memory context", "解释最近或指定 run 的召回结果", "memory", accepts_arguments=True),
            CommandSpec("/memory cleanup", "清理过期索引和候选正文", "memory"),
            CommandSpec("/memory embeddings", "配置或重建本地嵌入", "memory", accepts_arguments=True),
        ),
        accepts_arguments=True,
    ),
    CommandSpec(
