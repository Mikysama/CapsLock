"""Typed configuration resolution with environment precedence."""
...
class Settings:
    ...
    def load(
        ...
        return SettingsResolver(cls._resolve).resolve(workspace, layout=layout)
...
def load_config_document(path: Path, *, migrate: bool = False) -> dict[str, object]:
    return ConfigurationPipeline(
        validator=DocumentValidator(validate_config_document),
        migrator=DocumentMigrator(migrate_config),
    ).load(path, migrate=migrate, current_version=CONFIG_VERSION)
