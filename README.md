...
CapsLock 是一个本机工作区 Agent，用于读取代码、检索证据、检查 Git、提出受控文件修改、执行固定命令，以及按审批策略访问 Web 和本地 MCP。运行内核完全异步，运行、动作、审批、记忆和审计分别通过强类型领域接口与 SQLite repository 管理。

当前源码版本为 `2.2.4`。本次升级重构运行内核、token 上下文、工具 artifact、事件总线、SQLite 并发和插件隔离边界；只接受当前配置、数据库、archive、JSONL 与插件协议，不提供旧格式转换或兼容入口。架构与部署边界见 [当前开发者文档](docs/development/v2/current.md)，发布摘要见 [2.2.4 发布说明](docs/releases/v2.2.4.md)。
...
每个 run 只产生一个终止事件。终止类型为 `completed`、`waiting_approval`、`failed`、`cancelled` 或 `stopped`。完成事件携带 answer、citations、memory recalls、usage 和 duration；等待审批事件携带 action IDs；失败、取消与停止事件携带稳定的 error code 和 message。
...
插件包是包含 `capslock-plugin.toml` 和包内入口程序的本地目录。manifest、stdio protocol 与 workspace grant 使用当前协议，capability 必须声明工作区路径、网络 host、固定进程模板或 credential 名称。CapsLock 不支持在线源、自动依赖安装、市场、UI、模型 Provider、Hook 或后台任务。
...
插件安装到 `${CAPSLOCK_HOME:-~/.capslock}/plugins/`，工作区授权保存在 `.capslock/local/plugins.json`。模型可见工具名使用 `plugin_<plugin_name>_<tool_name>`，每次调用继续经过外部动作审批、超时、取消、脱敏和审计链。`trusted-native` 必须逐工作区显式确认，且每次调用仍要求人工审批。
...
配置根必须包含 `config_version = 3`。非当前配置直接拒绝，不自动迁移。多模型使用 provider、credential reference、profile 和角色路由：
...
工作区库和记忆库使用不同的 SQLite `application_id`。当前 workspace schema 为 6，memory schema 为 3；旧 application ID、非当前 schema 或其他已有表均拒绝启动且不会修改原文件。
