...
def test_agent_reads_project_layout_but_never_private_or_state(tmp_path: Path) -> None:
    ...
    root.joinpath("skills/demo/instructions.md").write_text("visible skill needle", encoding="utf-8")
    ...
    assert paths >= {
        ...
        ".capslock/mcp.json",
        ".capslock/skills/demo/instructions.md",
        ".env.example",
    }
    ...
    assert [item["path"] for item in searched.data] == [
        str(root.joinpath("skills/demo/instructions.md").resolve())
    ]
...
def test_agent_writes_only_project_skill_content_inside_capslock(tmp_path: Path) -> None:
    ...
    assert policy.writable_file(
        ".capslock/skills/demo/instructions.md", create=True
    ) == tmp_path.joinpath(".capslock/skills/demo/instructions.md")
    for path in (
        ".capslock/config.toml",
        ".capslock/mcp.json",
...
def test_skill_file_changes_always_require_explicit_approval(tmp_path: Path, mode: PermissionMode) -> None:
    path = tmp_path / ".capslock/skills/demo/instructions.md"
    ...
    result, _ = workspace_tools().invoke(
        "propose_file_edit",
        context,
        {"path": ".capslock/skills/demo/instructions.md", "old_text": "before", "new_text": "after"},
    )
