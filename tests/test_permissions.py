...
def test_full_access_auto_applies_edit_with_audit_events(tmp_path: Path) -> None:
    ...
    context = RunContext("session", "run", WorkspacePolicy(tmp_path), 6, lambda kind, **data: events.append((kind, data)), store, permission_mode=PermissionMode.FULL_ACCESS)
...
def test_permissions_menu_selects_and_persists_mode(tmp_path: Path, monkeypatch) -> None:
    ...
    cli._permissions(agent, "/permissions")
...
def test_command_tree_renders_claude_style_flat_two_column_list(monkeypatch) -> None:
    ...
    cli._render_command_tree("/permissions")
...
def test_completion_menu_is_pinned_to_left_edge() -> None:
    ...
    cli._anchor_completion_menus(root)
...
def test_prompt_has_history_separator_and_permission_rprompt() -> None:
