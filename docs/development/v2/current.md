...
当前格式为 config 10、workspace schema 17、memory schema 5、portable archive 6、session export 6、JSONL schema 3、IDE Bridge protocol 1 和 plugin protocol 4。workspace 启动支持 backup-first 的 v6-v16→v17 升级；重建表的迁移必须显式列出源、目标字段，禁止依赖物理列顺序。memory schema v3-v4 与 config v3-v9 自动备份并升级。迁移失败保留原库和备份，不继续部分升级。
